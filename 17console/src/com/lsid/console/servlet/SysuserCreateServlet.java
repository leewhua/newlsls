...
public class SysuserCreateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req,
		...
		try{
			...
			if (password==null||password.trim().isEmpty()){
				throw new Exception("passwordneeded");
			}
			...
			String newself = UUID.randomUUID().toString().replaceAll("-", "")+AutoConfig.SPLIT+request.getAttribute(request.getParameter("t0ken")+"self").toString().substring(0, request.getAttribute(request.getParameter("t0ken")+"self").toString().indexOf(AutoConfig.SPLIT))+
			...
			DataFiles.changecontent(URLEncoder.encode(request.getParameter("name"),"UTF-8"), URLEncoder.encode(request.getParameter("name"),"UTF-8"), 
					SecureFilter.aesdec(password,request.getAttribute(request.getParameter("t0ken")+"key").toString()), 
					newself);
