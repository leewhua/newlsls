...
public class ChinaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req,
		...
		try{
			...
			List<Map<String, Object>> data = DataFiles.luckchina(result.get("eid").toString(), request.getParameter("from"), request.getParameter("pc"), 
			...
			if ("luckuser".equals(request.getParameter("datatype"))) {
				data = DataFiles.luckuserchina(result.get("eid").toString(), request.getParameter("from"), request.getParameter("pc"), 
						request.getParameter("begin"), request.getParameter("end"), request.getParameter("gender"), 
						request.getParameter("activity"), request.getParameter("product"));
			} else if ("confirmprize".equals(request.getParameter("datatype"))) {
				data = DataFiles.confirmprizechina(result.get("eid").toString(), request.getParameter("from"), request.getParameter("pc"), 
						request.getParameter("begin"), request.getParameter("end"), request.getParameter("gender"), 
						request.getParameter("activity"), request.getParameter("product"));
			}
			result.putAll(data.remove(data.size()-1));
