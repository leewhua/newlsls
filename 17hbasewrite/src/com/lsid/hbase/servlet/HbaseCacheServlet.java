package com.lsid.hbase.servlet;
...
import com.lsid.cache.HbaseCache4Later;

@SuppressWarnings("serial")
public class HbaseCacheServlet extends HttpServlet {
		...
		try{
			...
			if (amount!=null&&!amount.trim().isEmpty()){
				try{
					...
					HbaseCache4Later.later(namespace, tablename, column, hash, row, amountnum);
				}catch(Exception ex){
					AutoConfig.innerechno(response, "amountnumneeded");
					return;
				}
			} else {
				HbaseCache4Later.later(namespace, tablename, column, hash, row, value);
			}
